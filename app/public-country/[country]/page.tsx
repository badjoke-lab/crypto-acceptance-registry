...
type PublicCountryDetailPageProps = {
  params: {
    country: string
  }
}
...
export default function PublicCountryDetailPage({ params }: PublicCountryDetailPageProps) {
  const country = decodeURIComponent(params.country)
  const merchants = getGeneratedProductMerchants()
    .filter((merchant) => (merchant.country || '').toLowerCase() === country.toLowerCase())
    .slice(0, 300)

  return (
    <main style={{ padding: 24, maxWidth: 1100, margin: '0 auto' }}>
      <p><Link href="/public-countries">← Back to public countries</Link></p>
      <h1>Country: {country}</h1>
      <p style={{ color: '#4b5563' }}>Ready merchants in this country: {merchants.length}</p>
