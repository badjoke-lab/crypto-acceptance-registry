...
function normalizeAccepted(input: RawPlace['accepted']): string[] {
  if (!input) return []
...
const outFile = path.join(process.cwd(), 'data', 'normalized-candidates-v2.json')
...
const normalized: NormalizedCandidateRecord[] = places
  ...
  .map((place) => {
    ...
    const acceptedAssets = uniqueNonEmpty([
      ...
      ...normalizeAccepted(place.accepted),
    ])
...
console.log(`normalized candidates v2: ${normalized.length}`)
